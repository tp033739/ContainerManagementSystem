...
using ContainerManagementSystem.Models.AccountViewModels;
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var r = await _userManager.AddToRoleAsync(user, model.IdentityRole);
                    if (r.Succeeded)
                    {
                        _logger.LogInformation($"User added to role {model.IdentityRole}");
                        return RedirectToLocal(returnUrl);
                    }
                    AddErrors(result);

                    // if we reached here, add to role must have failed, delete the user
